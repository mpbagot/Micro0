...
module gantry() {
    ...
    // Top extrusions
    translate([-5, -3, 43]) rotate([-90, 0, 0]) extrusion(EXTRUSION_LEN);
    translate([EXTRUSION_LEN+5, -3, 43]) rotate([-90, 0, 0]) extrusion(EXTRUSION_LEN);
}
...
module l_idler() {
    ...
    color(MAIN) difference() {
        ...
        hull() {
            translate([0, 0, 26]) cube([10, /*23*/17, 43-26]);
            translate([0, 17, 26]) cube([1.5, 6, 43-26]);
        }
        
        // Bolt holes
        // Front screw (M3)
        translate([5, 5, -5]) cylinder(r=1.7, h=100);
        translate([5, 5, 28]) cylinder(r=3, h=100);
        // Rear screw (maybe M3)
        ...
        // Idler cam slot
        translate([5, -0.01, 38]) rotate([-90, 0, 0]) cylinder(r=4, h=8);
    }
    
    // Bottom half
    color(MAIN) difference() {
        hull() {
            translate([0, 0, 0]) cube([10, /*23*/17, 26]);
            translate([0, 17, 0]) cube([1.5, 6, 26]);
        }
        ...
        // Bearing stack space
        translate([-1, -1, 17.001]) cube([12, 11, 9]);
        
        // Belt spaces
        translate([-0.01, 9.99, 16.99]) cube([2, 14, 10]);
        translate([7.99, 9.99, 16.99]) cube([2.1, 14, 10]);
    }
    ...
    translate([5, 5, 17]) bearing_stack();
}
...
module r_idler() {
    ...
    color(MAIN) difference() {
        ...
        hull() {
            translate([0, 0, 17]) cube([10, /*23*/17, 43-17]);
            translate([8.5, 17, 17]) cube([1.5, 6, 43-17]);
        }
    ...
    color(MAIN) difference() {
        ...
        hull() {
            translate([0, 0, 0]) cube([10, /*23*/17, 17]);
            translate([8.5, 17, 0]) cube([1.5, 6, 17]);
        }
        ...
        // Belt spaces
        translate([-0.01, 9.99, 7.99]) cube([2, 14, 10]);
        translate([7.99, 9.99, 7.99]) cube([2.1, 14, 10]);
    }
    // Belt stacks are centred in the space starting 1mm up from the bottom of the idler (i.e. the carriage overhang)
    // Space is thus 32mm from top to bottom.
    // Given that bearing stacks are 9mm each, 
    translate([5, 5, 8]) bearing_stack();
}
...
module x_axis() {
    ...
    // Rail
    rail_len = 120;
    translate([(EXTRUSION_LEN-rail_len)/2, MGN5_CARR_LEN-8, 3]) mgn5_rail(rail_len, x_pos);
...
module l_xy_joint() {
    ...
    color(MAIN) difference() {
        union() {
            cube([3, MGN5_CARR_LEN, 12+7]);
            
            
            translate([2, MGN5_CARR_LEN/3, 0]) cube([2, MGN5_CARR_LEN/3.5, 12]);
        
            hull() {
                translate([-14, 0, 12]) cube([14, MGN5_CARR_LEN, 7]);
                translate([-5.5, -6, 12]) cube([8.5, 6, 7]);
            }
            
           translate([-6, 6, 14]) cube([(EXTRUSION_LEN-120)/2, 10, 5]);
    ...
    color(MAIN) difference() {
        union() {
           translate([-6, 6, 29]) cube([(EXTRUSION_LEN-120)/2, 10, 14.5]);
            
            hull() {
                translate([-14, 0, 37]) cube([17, MGN5_CARR_LEN, 6.5]);
                translate([-5.5, -6, 37]) cube([8.5, 6, 6.5]);
            }
        }
        
        // Screw holes
        translate([-11, MGN5_CARR_LEN - 10 + 2.5, -10]) cylinder(r=1.7, h=100);
        translate([-11, MGN5_CARR_LEN - 10 + 2.5, 39]) cylinder(r=3, h=10);
    ...
    // Bearing stacks
    translate([-11, MGN5_CARR_LEN - 10 + 2.5, 19]) bearing_stack();
    translate([-11 + 7 + 1.5, MGN5_CARR_LEN - 10 + 2.5 - 7 - 1, 28]) bearing_stack();
    
    // Spacers
    translate([-11, MGN5_CARR_LEN - 10 + 2.5, 28]) spacer();
    translate([-11 + 7 + 1.5, MGN5_CARR_LEN - 10 + 2.5 - 7 - 1, 19]) spacer();
}
...
module drive_block() {
    ...
    color(MAIN) difference() {
        union() {
            translate([0, 2, 26]) cube([55, MOTOR_SIZE+8, 7]);
            translate([0, MOTOR_SIZE, 33]) cube([10, 10, 10]);
        }
        ...
        // Bearing stack screw holes
        translate([5, MOTOR_SIZE+5, 30])cylinder(r=3.5, h=100);
        translate([5, MOTOR_SIZE+5, 20]) cylinder(r=1.7, h=100);
        translate([5, MOTOR_SIZE+5-22.5, 30]) cylinder(r=3, h=100);
        translate([5, MOTOR_SIZE+5-22.5, 20]) cylinder(r=1.7, h=100);
        
        // Extrusion screw hole
        translate([5, MOTOR_SIZE+1.5, 38]) rotate([-90, 0, 0]) cylinder(r=3.5, h=100);
        translate([5, MOTOR_SIZE-2, 38]) rotate([-90, 0, 0]) cylinder(r=1.7, h=100);
        
        // Thread insert hole
        translate([5, MOTOR_SIZE+5, 31]) rotate([-90, 0, 0]) cylinder(r=2.4, h=7);

        // Connecting screw holes
        translate([5, 5, 30]) cylinder(r=3, h=100);
        translate([5, 5, 20]) cylinder(r=1.7, h=100);
        translate([47, 5, 30]) cylinder(r=3, h=100);
    ...
    color(MAIN) difference() {
        union() {
            translate([0, 0, 0]) cube([55, MOTOR_SIZE+10, 8]);
            translate([2, 0, 8]) cube([53, 10, 18]);
            translate([55 - 9.5, 0, 8]) cube([9.5, MOTOR_SIZE + 8, 18]); 
            
            // Alignment block
        translate([12.15, 3.15, 25.5]) cube([24.7, 5.7, 4.1]);
        }
        ...
        // TODO
        screw_size = 11;
        translate([55.01, MOTOR_SIZE/2 + 10 - screw_size/2, 33 - screw_size/2 - 2]) rotate([-90, 0, 90]) hull() {
    ...
    // Bearing stacks
    translate([5, MOTOR_SIZE+5, 8]) bearing_stack();
    translate([5, MOTOR_SIZE+5, 17]) bearing_stack();
    translate([5, MOTOR_SIZE+5-22.5, 8]) spacer();
    translate([5, MOTOR_SIZE+5-22.5, 17]) bearing_stack();

    // Tensioner bolt
    translate([250, -83.5, 26.2]) rotate([180, 0, 180]) color(ACCENT) import("STLs_to_mod/drive_tension_bolt.stl");
}

module carriage() {
    difference() {
        union() {
            translate([251.5, -30, 155.8]) rotate([-90, 90]) import("STLs_to_mod/carriage.stl");
        
            translate([0, 7, 0]) cube([MGN5_CARR_LEN, 12, 7]);
        }
        ...
        union() {
            ...
            translate([MGN5_CARR_LEN/2, 9, -1]) cylinder(r=1.2, h=100);
